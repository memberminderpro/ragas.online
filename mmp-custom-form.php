...
// Add custom rewrite rules based on current option values
function mmp_custom_rewrite_rules() {
    
    $settings = get_option('mmp_custom_form_settings');
    $accountID = isset($settings['AccountID']) ? esc_attr($settings['AccountID']) : null;
    $bid = isset($settings['BID']) ? esc_attr($settings['BID']) : null;

    // Check if the current accountID and bid match the desired values
    if ($accountID == '10651' && $bid == '1065') {
        // Add the custom rewrite rules
        add_rewrite_rule(
            '^application/contributor/?$',
            'index.php?pagename=application&mtid=933',
            'top'
        );

        add_rewrite_rule(
            '^application/supporter/?$',
            'index.php?pagename=application&mtid=938',
            'top'
        );

        add_rewrite_rule(
            '^application/rotaractor/?$',
            'index.php?pagename=application&mtid=935',
            'top'
        );
    }
}
add_action('init', 'mmp_custom_rewrite_rules');
...
function mmp_custom_rewrite_rules() {
    ...
    if ($accountID == '10651' && $bid == '1065') {
        // Add the custom rewrite rules
        add_rewrite_rule(
            '^application/(contributor|supporter|rotaractor)/?$',
            'index.php?pagename=application&mtid=$matches[1]',
            'top'
        );
        // Include the QSA flag to preserve the query string
        add_rewrite_rule(
            '^application/(contributor|supporter|rotaractor)/?$',
            'index.php?pagename=application&mtid=$matches[1]',
            'top,QSA'
